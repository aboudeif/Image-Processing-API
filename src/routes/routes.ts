  ...
  async (req: express.Request, res: express.Response): Promise<void> => {
    ...
    if (!fs.existsSync(info[0])) {
      ...
      response
        ...
        : res
            .status(500)
            .send(errormsg('image resize faild', 'could not resize image ' + info[1], 'try to resize it again'))
    } else {
      res.status(200).sendFile(info[0])
    }
  }
...
routes.get('/api/show', (req: express.Request, res: express.Response): void => {
  ...
  image
    ...
    : res
        .status(500)
        .send(errormsg('image not found', 'there is no image with name: ' + req.query.img, 'try to upload it first'))
})

// upload image to storage/images folder
routes.post('/upload', upload.single('uploaded_file'), async function (
  req: express.Request,
  ...
  response
    ? res.redirect('/')
    : res.status(500).send(errormsg('image upload faild', 'could not upload image', 'try to upload it again'))
})
...
routes.get('*', (req: express.Request, res: express.Response): void => {
  res
    .status(404)
    .send(errormsg('page not found', 'there is no path lead to here', `try to back to <a href='/'>home</a> page`))
})
